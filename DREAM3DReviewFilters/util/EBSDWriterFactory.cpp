...
{
	template <typename T> inline std::function<QString(QString, IDataArray::Pointer, uint64_t)> ArrayWriterFactory()
	{
		return WriteArrayToQStringLine<T>;
	}

	template <typename T> inline QString WriteArrayToQStringLine(QString rowData, IDataArray::Pointer ptr, uint64_t index)
	{
		...
		{
			typename T value = data->getValue(numComps*index + i);
	...
	template <> inline QString WriteArrayToQStringLine<int32_t>(QString rowData, IDataArray::Pointer ptr, uint64_t index)
	{
	...
	template <> inline QString WriteArrayToQStringLine<float>(QString rowData, IDataArray::Pointer ptr, uint64_t index)
	{
